...
const StorePage = () => {
  ...
  const { stores } = useSelector((state) => state.store); // <-- notice store here (not company)
  ...
  const handleCreateOrUpdateStore = async () => {
    ...
    try {
      if (isEditing && editingStoreId) {
        await dispatch(updateStore(editingStoreId, { name: storeName }));
        if (logoFile) {
        ...
        console.log("checking")
        await dispatch(createStore({ name: storeName,company:"680d39ef54b71a4d45d91072" }));
